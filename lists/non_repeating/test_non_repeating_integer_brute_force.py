...
from .non_repeating_integer_brute_force import find_first_unique
...
class TestFindFirstUnique(TestCase):
    ...
    def test_all_identical_elements(self):
        self.assertIsNone(find_first_unique([1, 1, 1, 1]))

    def test_negative_numbers(self):
        self.assertEqual(find_first_unique([-1, -2, -2, -3, -3]), -1)

    def test_mixed_data_types(self):
        self.assertEqual(find_first_unique([1, 'a', 1, 'b', 'a']), 'b')

    def test_very_large_numbers(self):
        self.assertEqual(find_first_unique([10**18, 10**18, 10**19]), 10**19)

    def test_special_characters(self):
        self.assertEqual(find_first_unique(['@', '#', '@', '$', '#']), '$')
